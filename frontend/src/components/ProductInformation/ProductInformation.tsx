...
const ProductInformation: React.FC = () => {
  ...
  const fetch = async () => {
    try {
      ...
      const valuePropositionsResponse = await getValuePropositions({ conversationId: conversationId, question: templete2(selectedboat || "") })
      const walkaroundResponse = await getWalkthroughData({ conversationId: conversationId, question: templete3(selectedboat || "") })

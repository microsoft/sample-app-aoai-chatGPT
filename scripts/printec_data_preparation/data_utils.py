...
def downloadBlobUrlToLocalFolder(blob_url, local_data_folder, credential):
    # Extract storage details from the URL
    storage_account, container_name, blob_name = extractStorageDetailsFromUrl(blob_url)
    ...
    local_file_path = os.path.join(local_data_folder, blob_name.split('/')[-1])
...
def chunk_blob_container(
    ...
    with tempfile.TemporaryDirectory() as local_data_folder:
        ...
        downloadBlobUrlToLocalFolder(blob_url, local_data_folder, sas_token)
        print(f'Downloaded.')

...
from quart import (Blueprint, Quart, abort, current_app, jsonify,
...
def check_authorization():
    ...
    if access_token:
        if app_settings.auth_sharepoint.enabled and not ms_graph_authorisation_check(
            access_token, app_settings.auth_sharepoint.site_id, app_settings.auth_sharepoint.page_id
        ):
            abort(401)
...
frontend_settings = {
    ...
    "ui": {
        "title": app_settings.ui.title,
        "hugo_url": app_settings.ui.hugo_url,
        ...
        "citations_expanded": app_settings.ui.citations_expanded,
        "citations_as_links": app_settings.ui.citations_as_links,
        "out_of_scope_message": app_settings.ui.out_of_scope_message,
    },
...
async def init_openai_client():
    ...
    try:
        ...
        if (
            ...
            raise ValueError(
                f"The minimum supported Azure OpenAI preview API version is '{MINIMUM_SUPPORTED_AZURE_OPENAI_PREVIEW_API_VERSION}' currently set to '{app_settings.azure_openai.preview_api_version}'"
            )

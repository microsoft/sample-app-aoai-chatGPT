...
AZURE_COSMOSDB_MONGO_VCORE_CONNECTION_STRING = os.environ.get(
    "AZURE_COSMOSDB_MONGO_VCORE_CONNECTION_STRING")  # This has to be secure string
...
def stream_without_data(response, history_metadata={}):
    ...
    for line in response:
        if line["choices"] and "content" in line["choices"][0]["delta"]:
...
def get_conversation():
    ...
    messages = [{'id': msg['id'], 'role': msg['role'], 'content': msg['content'],
                 'createdAt': msg['createdAt'], 'feedback': msg.get('feedback')} for msg in conversation_messages]
...
def generate_title(conversation_messages):
    ...
    try:
        ...
        title = json.loads(completion['choices']
                           [0]['message']['content'])['title']
        return title
    except Exception as e:

...
|2|SharePointを検索対象にした導入事例を探してください|2社の事例を回答するか確認|2社の事例を回答|2社の事例を表示|
|3|SPOを検索対象にした導入事例を探してください|「SPO」を「SharePoint Online」として回答できるか確認（あいまい検索）|2社の事例を回答|類義語検索で2社の事例を表示|
...
- 比較のために、Difyで作ったチャットボットも利用する。LLMやEmbeddingはAzure OpenAI Serviceを利用する。Rerankモデルはcohereの`rerank-multilingual-v3.0`。Difyの方は、8ファイルをアップロード。実験内容によってチャキングを変更したものを使用。
...
|LLMのモデルの違いによる回答精度の違いと回答速度の違い|LLM||2|||
|LLMのパラメーターの影響|LLM|||||

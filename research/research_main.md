...
|--:|:---|:--|:--------|:-------|
|1|Neuronの導入事例を探してください|数社の事例を回答する|5社~7社の事例を回答|26社の事例を表示|
...
|4|Neuronの導入事例を10社分探してください|10社分の事例を回答する|7社を回答|結果に表示する事例の数は指定できないが10社以上の事例を表示|
|5|ニューロンの導入事例を探してください|カタカナになっても検索する|||
|6|Neruonの導入事例を探してください|typoでもNeuronの事例を検索する|||
...
|9|シナプスの導入事例を探してください|存在しない情報は回答しない||検索結果が出ない|
...
|Embeddingモデルの影響|ベクトル化| text-embedding-3-small<br>text-embedding-3-large |           |両モデルで精度は同じ| モデルによって精度は変わらないので、smallで十分 |
...
## 料金に関する考察
結論
- Azureでチャットボットを構築すると、ユーザー数による課金の要素は極めて少ないと考える。
- サービス利用料としては、検索対象ファイル数をもとにした月額固定の基本料金と、LLMなどの従量課金の利用料の発生が妥当と考えられる。
- セキュリティ対応や利用時間はお客様の要件に合わせられる方が良いと考える。
- 会話履歴の抽出には料金が発生しない方が望ましいと考える。

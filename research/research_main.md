...
| [Embeddingモデルの影響](./research_embedding.md) |ベクトル化| text-embedding-3-small<br>text-embedding-3-large |           | チャンク長が同一の場合、基本検索性能はほぼ同等だが、カタカナ検索ではtext-embedding-3-largeがわずかに優位| リソース効率とコストを考慮すると、基本的な検索にはsmallモデルで十分だが、日本語の表記揺れに対応する必要がある場合はlargeモデルの採用を検討 |
...
- 夜間や土日など、チャットボットを利用しない時間も課金される。Azure AI SearchのかかるレベルをBにするなら月額$330なので、4コアの仮想サーバーの方が低コストになる可能性ある。

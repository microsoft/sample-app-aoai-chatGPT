...
|9|シナプスの導入事例を探してください|存在しない情報に対する挙動を確認|
|10|製造業が抱える課題について教えて||
...
- 実験内容によってはDifyで作ったチャットボットを利用します。LLMやEmbeddingはAzure OpenAI Serviceを利用し、Rerankモデルはcohereの`rerank-v3.5`を利用します。ナレッジは同じ26事例のファイルを使います。
